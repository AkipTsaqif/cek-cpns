...
export default async function Home({
    ...
    const { data: tingkat_pendidikan, error } = await supabase
        .from("distinct_jenjang_pendidikan")
        .select("*");
    ...
    const formasi = formasiAwal
        ? await Promise.all(
              formasiAwal.map((item) => getFormasi(item.cepat_kode))
          )
        : [];
    const formasiDetail = formasi
    ...
    return (
        <div className="flex flex-col p-6 gap-3">
            <h1 className="text-2xl font-bold">
                Cek Formasi CPNS {new Date().getFullYear()}
            </h1>
            <MainComponent
